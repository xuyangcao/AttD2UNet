...
class SEBlock(nn.Module):
    def __init__(self, in_channels, reduction_ratio=None, **kwargs):
        super(SEBlock, self).__init__()
        if reduction_ratio is not None:
            self.reduction_ratio = reduction_ratio
        else:
            self.reduction_ratio = in_channels // 6

        self.avg_pool = nn.AdaptiveAvgPool3d(1)
        self.mlp = nn.Sequential(
                nn.Linear(in_channels, in_channels // self.reduction_ratio, bias=False),
                nn.ReLU(inplace=True),
                nn.Linear(in_channels // self.reduction_ratio, in_channels, bias=False)
                )
        self.relu = nn.ReLU()

    def forward(self, x):
        x_pool = self.avg_pool(x).squeeze()
        scale = self.mlp(x_pool)
        scale = torch.sigmoid(scale).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand_as(x)
...
class SpatialAttention(nn.Module): 
    def __init__(self, kernel_size=3, padding=1, stride=1, dilation=1, groups=1, bias=False, act_func='relu', **kwargs):
        ...
        self.spatial = nn.Conv3d(2, 1, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=bias)
        self.act = nn.PReLU()
        self.relu = nn.ReLU()
...
class ChannelAttention(nn.Module): 
    def __init__(self, in_channels, reduction_ratio=None, pool_types=['avg', 'max'], **kwargs):
        super().__init__()
        self.gate_channels = in_channels
        if reduction_ratio is not None:
            self.reduction_ratio = reduction_ratio
        else:
            self.reduction_ratio = gate_channels // 6
